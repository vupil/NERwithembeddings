...
with open('AllClasses.lst') as f:
    for line in f:
        ...
        if GO_class in word_vectors.vocab:
            ...
            # remove commas (',') and replace with no space to handle class names with commas while writing to csv
            array_str = str(GOvectors[GO_class].tolist())
            file.write (str(GO_class).replace(",","") + ','+ array_str + '\n') # tolist() to save entire array in a single row
